...
def override_auth():
    app.dependency_overrides[get_current_user] = lambda: {"id": 1, "login": "current_user"}
    yield
...
async def test_add_friendship_success():
    with patch("app.services.friend.get_user_by_login", new_callable=AsyncMock) as mock_get, \
         patch("app.services.friend.is_friendship_exists", new_callable=AsyncMock) as mock_exists, \
         patch("app.services.friend.add_users_friendship", new_callable=AsyncMock) as mock_add:
...
async def test_add_friendship_same_user():
    ...
    with patch("app.services.friend.get_user_by_login", new_callable=AsyncMock) as mock_get:
        ...
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            response = await client.post("/api/v1/friendship", json={"user_login": "current_user"})
...
async def test_accept_friend_success():
    with patch("app.services.friend.get_user_by_login", new_callable=AsyncMock) as mock_get, \
         patch("app.services.friend.accept_friendship", new_callable=AsyncMock) as mock_accept:
