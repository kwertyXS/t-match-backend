...
FAKE_PROFILE = type("Profile", (), {
    "id": 1, "user_id": 1, "title": "My Profile",
    "description": "Test desc", "tags": ["python", "fastapi"]
})()

CREATE_PAYLOAD = {"title": "My Profile", "description": "Test desc", "tags": ["python", "fastapi"]}
UPDATE_PAYLOAD = {"id": 1, "title": "Updated", "description": "New desc", "tags": ["updated"]}
...
async def test_get_profile_success():
    with patch("app.services.profile.get_user_profile", new_callable=AsyncMock) as mock_get:
...
async def test_get_user_profiles_success():
    with patch("app.services.profile.get_user_by_login", new_callable=AsyncMock) as mock_user, \
         patch("app.services.profile.get_profiles", new_callable=AsyncMock) as mock_profiles:
...
async def test_new_profile_title_too_short():
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        response = await client.post("/api/v1/profile", json={
            "title": "ab", "description": "desc", "tags": []
        })
...
async def test_new_profile_title_too_long():
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        response = await client.post("/api/v1/profile", json={
            "title": "a" * 31, "description": "desc", "tags": []
        })
...
async def test_edit_profile_success():
    with patch("app.services.profile.update_profile", new_callable=AsyncMock) as mock_update:
        mock_update.return_value = type("Profile", (), {
    ...
    assert response.status_code == 200
    assert response.json() == {"title": "Updated", "description": "New desc", "tags": ["updated"]}