...
def override_auth():
    app.dependency_overrides[get_current_user] = lambda: {"id": 42, "username": "tester"}
    yield
...
async def test_new_meet_success():
    with patch("app.services.meet.add_meet", new_callable=AsyncMock) as mock_add:  # ← реальный путь
        mock_add.return_value = FakeMeeting()

        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            response = await client.post("api/v1/meeting", json=VALID_PAYLOAD)  # ← реальный URL

    assert response.status_code == 200
    assert response.json() == {"id": 1, "title": "Планёрка", "description": "Обсуждение задач"}
...
async def test_new_meet_invalid_payload():
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        response = await client.post("api/v1/meeting", json={"title": "только заголовок"})
